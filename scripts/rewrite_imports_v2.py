...
def rewrite_file(
    ...
    try:
        ...
        sorted_patterns = sorted(
            conversion_map.items(), key=lambda x: len(x[0]), reverse=True
        )
...
def rewrite_modules(
    ...
    for module_dir in sorted(module_dirs):
        py_files = [
            f for f in module_dir.glob("*.py") if not f.name.startswith("__init__")
        ]
...
def main():
    ...
    parser.add_argument(
        "--modules", help="Module pattern (e.g., 'core-*' or 'core-*,error-*')"
    )
