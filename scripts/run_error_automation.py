...
def cmd_apply_patch(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            f"  Tests:       {'✅' if conf['tests_passed'] == 1.0 else '❌'} {conf['tests_passed']:.1%}"
        )
        print(
            f"  Linting:     {'✅' if conf['lint_passed'] == 1.0 else '⚠️'} {conf['lint_passed']:.1%}"
        )
        print(
            f"  Type Check:  {'✅' if conf['type_check_passed'] == 1.0 else '⚠️'} {conf['type_check_passed']:.1%}"
        )
        print(
            f"  Security:    {'✅' if conf['security_scan_passed'] == 1.0 else '❌'} {conf['security_scan_passed']:.1%}"
        )
        print(
            f"  Coverage:    {'✅' if conf['coverage_maintained'] >= 0.7 else '⚠️'} {conf['coverage_maintained']:.1%}"
        )
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            f"  ✅ Auto-merged:    {data['auto_merged']} ({data['auto_merged']/total*100:.1f}%)"
        )
        print(
            f"  🔄 PR created:     {data['pr_created']} ({data['pr_created']/total*100:.1f}%)"
        )
        print(
            f"  👁️  Manual review:  {data['manual_review']} ({data['manual_review']/total*100:.1f}%)"
        )
        print(
            f"  ❌ Rejected:       {data['rejected']} ({data['rejected']/total*100:.1f}%)"
        )
...
Examples:
  ...
  %(prog)s status --days 7
    ...
    apply_parser.add_argument(
        "patch_path", type=str, help="Path to patch file (.patch or .diff)"
    )
    apply_parser.add_argument(
    ...
    queue_parser = subparsers.add_parser(
        "process-queue", help="Process manual review queue"
    )
    ...
    queue_parser.add_argument(
        "--patch-id", type=str, help="Patch identifier (required for approve/reject)"
    )
    queue_parser.add_argument(
        "--reason", type=str, help="Rejection reason (optional for reject action)"
    )
    queue_parser.add_argument(
        "--min-confidence",
        type=float,
    ...
    # Status command
    status_parser = subparsers.add_parser("status", help="Show error automation status")
