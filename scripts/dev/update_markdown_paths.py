...
MOVED_DIRS_MAP = {
    "PHASE_DEV_DOCS": _resolve_or_default("docs.phase_dev_root", "meta/PHASE_DEV_DOCS"),
    "plans": _resolve_or_default("docs.plans_root", "meta/plans"),
    "Coordination Mechanisms": _resolve_or_default(
        "docs.coord_mech_root", "meta/Coordination Mechanisms"
    ),
}
...
for new_abs_path_obj in all_meta_md_files:
    new_rel_path = str(new_abs_path_obj.relative_to(REPO_ROOT)).replace(
        "\\", "/"
...
print(
    f"DEBUG: old_to_new_abs_path_map keys: {list(old_to_new_abs_path_map.keys())[:5]}..."
)  # Print first 5 keys
print(
    f"DEBUG: new_to_old_abs_path_map keys: {list(new_to_old_abs_path_map.keys())[:5]}..."
)  # Print first 5 keys
...
def update_markdown_paths():
    for new_abs_file_path in all_meta_md_files:
        ...
        if not original_abs_file_path_str:
            print(
                f"Warning: Could not find original path for {new_abs_file_path}. Skipping."
            )
            continue
        ...
        for match in MARKDOWN_LINK_REGEX.finditer(content):
            ...
            if link_path.startswith("/"):
                ...
                resolved_old_linked_path = str(
                    original_abs_file_path.parent / link_path
                )
            ...
            if resolved_old_linked_path in old_to_new_abs_path_map:
                new_linked_abs_path = Path(
                    old_to_new_abs_path_map[resolved_old_linked_path]
                )

                # Calculate new relative path from the current file's new location to the linked file's new location
                try:
                    new_relative_link_path = os.path.relpath(
                        new_linked_abs_path, new_abs_file_path.parent
                    )
                    ...
                    updated_content = updated_content.replace(
                        f"({link_path})", f"({new_relative_link_path})"
                    )
                except ValueError as e:
                    print(
                        f"Error calculating relative path for {new_abs_file_path} to {new_linked_abs_path}: {e}"
                    )
                    continue
            else:
                print(
                    f"DEBUG: Linked file {resolved_old_linked_path} not found in old_to_new_abs_path_map."
                )
