...
class DocTriageResult:
    ...
    def __init__(
        self, path: Path, category: str, issue: str = None, suggestion: str = None
    ):
        self.path = path
...
class DocTriage:
    ...
    def __init__(self, scan_path: Path = REPO_ROOT):
        self.scan_path = (
            scan_path.resolve() if not scan_path.is_absolute() else scan_path
        )
    ...
    def report(self, report_only: bool = False):
        ...
        print(
            f"Files needing action: {len(self.results) - len(by_category.get('ok', []))}"
        )
        print()
...
def main():
    ...
    parser.add_argument(
        "--path", type=Path, default=REPO_ROOT, help="Path to scan (default: repo root)"
    )
    parser.add_argument(
        "--report-only",
        action="store_true",
