...
# Summarize and check exit code
EXIT_CODE=$?

# Best-effort summary for pytest logs
TOTAL=0; PASSED=0; FAILED=0
if grep -qE "==+ .* in " "$LOG_FILE" 2>/dev/null; then
  SUMMARY_LINE=$(grep -E "==+ .* in " "$LOG_FILE" | tail -n1)
  PASSED=$(echo "$SUMMARY_LINE" | sed -n 's/.*\b\([0-9][0-9]*\) passed.*/\1/p')
  FAILED=$(echo "$SUMMARY_LINE" | sed -n 's/.*\b\([0-9][0-9]*\) failed.*/\1/p')
  [ -z "$PASSED" ] && PASSED=0
  [ -z "$FAILED" ] && FAILED=0
  TOTAL=$((PASSED + FAILED))
fi

# Emit stable summary marker for parsers
echo "CCPM_TEST_SUMMARY total=$TOTAL passed=$PASSED failed=$FAILED exit=$EXIT_CODE"
