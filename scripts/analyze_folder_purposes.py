...
class FolderProfile:
    ...
    def similarity_score(self, other: 'FolderProfile') -> int:
        ...
        type_diff = abs(self.py_count - other.py_count) + \
        ...
        if self.file_count > 0 and other.file_count > 0:
            size_ratio = min(self.file_count, other.file_count) / max(self.file_count, other.file_count)
...
def determine_scope(folder_path: str) -> str:
    ...
    if any(tool in path_lower for tool in ['ai-logs-analyzer', 'glossary', 'aim', 'aider', 'ccpm']):
        return 'tool-specific'
    
    # Module-specific (subdirectory of a module)
    if any(mod in path_lower for mod in ['universal_execution_templates', 'core/', 'engine/', 'error/']):
...
def analyze_duplicate_folders(repo_root: Path) -> Dict:
    ...
    for folder in repo_root.rglob('*'):
        ...
        if any(excl in str(folder) for excl in ['.git', '.worktrees', '.venv', '__pycache__', 'node_modules']):
            continue
    ...
    for name, folders in all_folders.items():
        ...
        if scopes == {'global', 'tool-specific'} or scopes == {'global', 'module-specific'}:
