...
class ComprehensiveFileScore:
    ...
    def __post_init__(self):
        ...
        self.confidence = min(100, self.composite_score + (10 if high_signal_count >= 3 else 0))
...
class ComprehensiveArchivalAnalyzer:
    ...
    def __init__(self, root: Path, confidence_threshold: int, staleness_days: int, 
                 exclude_extensions: List[str], output_dir: Path):
        self.root = root
    ...
    def run_all_analyzers(self):
        ...
        self._run_analyzer(
            ...
            "--output", str(self.output_dir / "entry_point_reachability_report.json")
        )
        self.reachability_data = self._load_json(self.output_dir / "entry_point_reachability_report.json")
        
        # Step 2: Test Coverage
        logger.info("\nStep 2/5: Running test coverage analysis...")
        self._run_analyzer(
            "python", "scripts/test_coverage_archival.py",
            "--output", str(self.output_dir / "test_coverage_archival_report.json")
        )
        self.test_coverage_data = self._load_json(self.output_dir / "test_coverage_archival_report.json")
        ...
        self._run_analyzer(
            ...
            "--output", str(self.output_dir / "parallel_implementations_analysis.json")
        )
        self.parallel_impl_data = self._load_json(self.output_dir / "parallel_implementations_analysis.json")
        ...
        self._run_analyzer(
            ...
            "--exclude-extensions", ",".join(self.exclude_extensions),
    ...
    def _aggregate_signals(self):
        ...
        for file_path in all_files:
            ...
            reach_score = reach_data.get('score', 0) if isinstance(reach_data, dict) else 0
            ...
            if self.cleanup_data:
                ...
                if 'file_scores' in self.cleanup_data:
                    for file_score in self.cleanup_data['file_scores']:
                        if isinstance(file_score, dict) and file_score.get('path') == file_path:
    ...
    def generate_tiered_reports(self):
        ...
        report = {
            ...
            "signal_convergence": {
                "3_plus_signals": sum(1 for s in self.comprehensive_scores.values() 
                                     if sum([s.duplication_score >= 80, s.staleness_score >= 80,
                                            s.obsolescence_score >= 80, s.isolation_score >= 80,
                                            s.reachability_score >= 80, s.test_coverage_score >= 80]) >= 3)
            }
        }
    ...
    def _generate_tier1_script(self, tier1_files: List[Dict]):
        ...
        for i, file_data in enumerate(tier1_files, 1):
            ...
            script_lines.append(f"    Write-Host '[DRY RUN] Would archive: {file_path}' -ForegroundColor Yellow")
            script_lines.append("} else {")
            script_lines.append(f"    $Source = '{file_path}'")
            script_lines.append("    $Dest = Join-Path $ArchiveDir $Source")
            script_lines.append("    New-Item -ItemType Directory -Force -Path (Split-Path $Dest) | Out-Null")
            script_lines.append("    Move-Item -Path $Source -Destination $Dest -Force")
            script_lines.append(f"    Write-Host '✅ Archived: {file_path}' -ForegroundColor Green")
            script_lines.append("}")
            script_lines.append("")
    ...
    def print_summary(self, report: Dict):
        ...
        print(f"\nTiered Results:")
        print(f"  TIER 1 (90-100%): {stats['tier_1_count']:,} files - SAFE FOR AUTOMATED ARCHIVAL")
        print(f"  TIER 2 (75-89%):  {stats['tier_2_count']:,} files - REVIEW RECOMMENDED")
        print(f"  TIER 3 (60-74%):  {stats['tier_3_count']:,} files - MANUAL EXPERT REVIEW")
        print(f"  TIER 4 (<60%):    {stats['tier_4_count']:,} files - KEEP")
        
        print(f"\nSignal Convergence:")
        print(f"  3+ signals ≥80:   {report['signal_convergence']['3_plus_signals']:,} files")
        ...
        print("2. Review: cleanup_reports/parallel_implementations_decision_checklist.md")
        print("3. Validate: python scripts/validate_archival_safety.py --mode pre-archive")
        print("4. Execute (dry-run): .\\cleanup_reports\\archival_plan_tier1_automated.ps1")
