...
class ModuleIDAssigner:
    ...
    def infer_module_id(self, doc: Dict) -> Tuple[str, str]:
        ...
        if path.startswith('doc_id/') or (path.startswith('docs/') and category == 'guide'):
        ...
        if path.startswith('infra/') or path.startswith('.github/') or path.startswith('ci/'):
        ...
        if path.startswith('.claude/') or path.startswith('prompting/') or path.startswith('aider/'):
            return 'docs.tooling', 'path_match'
        ...
        if path.startswith('specifications/') or path.startswith('schema/') or path.startswith('openspec/'):
            return 'docs.guides', 'path_match'

        # Rule 15: Reports and state
        if path.startswith('reports/') or path.startswith('.state/') or path.startswith('state/'):
            return 'infra.ci', 'path_match'
        ...
        if path.startswith('assets/') or path.startswith('glossary/') or path.startswith('examples/'):
            return 'docs.guides', 'path_match'
        ...
        if path.startswith('modules/') or path.startswith('registry/') or path.startswith('tools/'):
            return 'docs.guides', 'path_match'

        # Rule 20: Refactor and hidden directories
        if path.startswith('REFACTOR_') or path.startswith('.ai') or path.startswith('.config') or path.startswith('.uet') or path.startswith('.execution'):
            return 'docs.guides', 'path_match'
    ...
    def infer_from_category(self, doc: Dict, category: str) -> Tuple[str, str]:
        ...
        reason = 'category_fallback' if module_id != 'unassigned' else 'no_artifact_no_category_match'
    ...
    def assign_module_ids(self, dry_run: bool = True) -> Dict:
        ...
        for i, doc in enumerate(docs, 1):
            ...
            if module_id == 'unassigned':
                self.unassigned.append({
                    'doc_id': doc_id,
                    'category': doc.get('category', 'unknown'),
                    'candidate_paths': [a.get('path', '') for a in doc.get('artifacts', [])],
                    'reason': reason
                })
            else:
                assigned += 1
        ...
        print(f"   Unassigned: {len(self.unassigned)} ({len(self.unassigned)*100//total}%)")
    ...
    def generate_reports(self):
        ...
        with open(dry_run_report, 'w', encoding='utf-8') as f:
            ...
            f.write("## Summary\n\n")
            f.write(f"- Total docs: {sum(len(docs) for docs in self.module_stats.values())}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Assign module_id to all docs in registry')
    parser.add_argument('--dry-run', action='store_true', help='Preview assignments without modifying registry')
    parser.add_argument('--apply', action='store_true', help='Apply assignments to registry')
    parser.add_argument('--report-only', action='store_true', help='Generate reports only (requires previous run)')
