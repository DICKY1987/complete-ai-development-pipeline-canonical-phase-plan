...
def sample_bundle():
    ...
    return {
        ...
        "ccpm_issue": 456,
        "gate": 1,
        "files_scope": ["src/module.py", "src/utils.py"],
        "files_create": ["tests/test_module.py"],
        "tasks": [
            "Refactor module.py for clarity",
            "Add unit tests",
            "Update documentation",
        ],
        "acceptance_tests": ["pytest -q tests/test_module.py", "ruff check src/"],
        "tool": "aider",
...
def test_classification_creation():
    ...
    c = Classification(
        complexity="moderate", quality="standard", domain="code", operation="refactor"
    )
...
def test_infer_classification_simple(prompt_engine):
    ...
    bundle = {"files_scope": ["src/file.py"], "tasks": ["Make small change"], "gate": 1}
...
def test_infer_role_simple_code(prompt_engine):
    ...
    classification = Classification(
        complexity="simple", quality="standard", domain="code", operation="refactor"
    )
...
def test_infer_role_senior_docs(prompt_engine):
    ...
    classification = Classification(
        complexity="complex", quality="production", domain="docs", operation="feature"
    )
...
def test_infer_role_principal_code(prompt_engine):
    ...
    classification = Classification(
        complexity="enterprise", quality="production", domain="code", operation="bugfix"
    )
...
def test_render_fallback(prompt_engine, sample_bundle, prompt_context):
    ...
    rendered = prompt_engine._render_fallback(
        sample_bundle, prompt_context, classification, role
    )
...
def test_classification_all_combinations(prompt_engine):
    ...
    for file_count, task_count, gate, exp_complexity, exp_quality in test_cases:
        ...
        assert (
            classification.complexity == exp_complexity
        ), f"Failed for {file_count} files, {task_count} tasks"
        assert classification.quality == exp_quality, f"Failed quality for gate={gate}"
