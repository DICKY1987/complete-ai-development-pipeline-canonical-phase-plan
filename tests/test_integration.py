...
import pytest
from modules.core_engine.m010001_pipeline_plus_orchestrator import (
    PipelinePlusOrchestrator,
)
...
def test_execute_task_unknown_adapter(orchestrator, tmp_path):
    task = Task(
        ...
        mode="prompt",
        capabilities=[],
        payload=TaskPayload(repo_path=str(tmp_path)),
    )
...
def test_task_queue_integration(orchestrator):
    task = Task(
        ...
        payload=TaskPayload(repo_path="/test"),
    )
