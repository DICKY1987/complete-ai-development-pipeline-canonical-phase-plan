...
def test_sqlite_pattern_store_reads_runs_and_events(tmp_path):
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        ("task-psql-1", "exec-psql-1", "PHASE_A", "[]", "completed", now, now, now, "{}"),
    )
    conn.execute(
        """
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        ("patch-psql-1", "exec-psql-1", now, "validated", "diff --git a/x b/x\n+1", "{}", "{}"),
    )
    conn.commit()
    ...
    assert run.status in (PatternStatus.RUNNING, PatternStatus.COMPLETED, PatternStatus.PENDING)
