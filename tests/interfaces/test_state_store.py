...
class TestSQLiteStateStore:
    ...
    def test_save_and_get_workstream(self, store):
        ...
        ws = {"id": "ws-test", "status": "pending", "tasks": ["Task 1", "Task 2"]}
    ...
    def test_list_executions(self, store):
        ...
        store.record_execution(
            {"run_id": "run-001", "ws_id": "ws-1", "status": "running"}
        )
        store.record_execution({"run_id": "run-001", "ws_id": "ws-2", "status": "done"})
    ...
    def test_list_executions_filtered(self, store):
        ...
        store.record_execution({"run_id": "run-001", "ws_id": "ws-2", "status": "done"})
        store.record_execution(
            {"run_id": "run-002", "ws_id": "ws-3", "status": "running"}
        )
    ...
    def test_update_execution_status(self, store):
        ...
        exec_id = store.record_execution(
            {"run_id": "run-001", "ws_id": "ws-test", "status": "running"}
        )

        store.update_execution_status(
            exec_id, "done", completed_at=datetime.now(), exit_code=0
        )
    ...
    def test_record_event(self, store):
        ...
        # Should not raise
        store.record_event(
            "job.completed", {"job_id": "job-001", "exit_code": 0, "duration_s": 12.5}
        )
