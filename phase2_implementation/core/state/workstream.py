...
class WorkstreamState(BaseState):
    ...
    def get_valid_transitions(cls) -> Dict['WorkstreamState', Set['WorkstreamState']]:
        ...
        return {
            ...
            cls.RUNNING: {cls.PAUSED, cls.BLOCKED, cls.VALIDATING, cls.FAILED, cls.CANCELLED},
...
class WorkstreamStateMachine(BaseStateMachine):
    ...
    def __init__(self, workstream_id: str, run_id: str, metadata: Optional[Dict] = None):
        super().__init__(
    ...
    def mark_ready(self):
        ...
        self.transition(WorkstreamState.READY, reason="Dependencies satisfied", trigger="ready")
    
    def start(self):
        """Start workstream execution."""
        self.transition(WorkstreamState.RUNNING, reason="Workstream started", trigger="started")
        self.started_at = datetime.now(timezone.utc)
    
    def pause(self, operator: str = None):
        """Pause workstream."""
        self.transition(WorkstreamState.PAUSED, reason="Paused", trigger="paused", operator=operator)
    
    def resume(self, operator: str = None):
        """Resume workstream."""
        self.transition(WorkstreamState.RUNNING, reason="Resumed", trigger="resumed", operator=operator)
    ...
    def unblock(self):
        """Unblock workstream."""
        self.transition(WorkstreamState.PENDING, reason="Unblocked", trigger="unblocked")
    
    def begin_validation(self):
        """Begin validation."""
        self.transition(WorkstreamState.VALIDATING, reason="All tasks complete", trigger="validating")
    
    def complete(self):
        """Mark as completed."""
        self.transition(WorkstreamState.COMPLETED, reason="Validation passed", trigger="completed")
    ...
    def cancel(self, operator: str = None):
        """Cancel workstream."""
        self.transition(WorkstreamState.CANCELLED, reason="Cancelled", trigger="cancelled", operator=operator)
        self.completed_at = datetime.now(timezone.utc)
    
    def update_task_counts(self, total: int = None, completed: int = None, failed: int = None):
        """Update task counts."""
    ...
    def get_progress(self) -> Dict:
        ...
        if self.total_tasks > 0:
            progress['completion_percentage'] = (self.completed_tasks / self.total_tasks) * 100
