...
class BaseStateMachine(ABC):
    ...
    def transition(
        ...
        if self.is_terminal():
            raise TerminalStateViolation(
                f"Cannot transition from terminal state: " f"{self.current_state.value}"
            )
        ...
        self.history.append((to_state, datetime.now(timezone.utc), reason or trigger))
    ...
    def get_state_history(self) -> List[Tuple[str, str, str]]:
        ...
        """
        return [
            (state.value, ts.isoformat(), reason) for state, ts, reason in self.history
        ]
