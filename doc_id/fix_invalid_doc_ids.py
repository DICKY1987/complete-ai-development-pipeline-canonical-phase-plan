...
PATTERN: PAT-DOC-ID-FIX-INVALID-001
...
# Valid doc_id pattern: DOC-PREFIX-NAME-###
VALID_DOC_ID_REGEX = re.compile(r"^DOC-[A-Z0-9]+-[A-Z0-9]+(-[A-Z0-9]+)*-[0-9]{3}$")
...
def generate_fixed_doc_id(invalid_doc_id: str) -> Tuple[str, str]:
    """Generate a valid doc_id by adding numeric suffix."""
    category = extract_category_from_doc_id(invalid_doc_id)
    next_id = get_next_id_for_category(category)
    
    # Add -XXX suffix
    fixed_doc_id = f"{invalid_doc_id}-{next_id:03d}"
