...
class BootstrapValidator:
    def __init__(
        self, project_profile_path: str, router_config_path: str, profile_id: str
    ):
        """Load artifacts to validate"""
        # DOC_ID: DOC-CORE-BOOTSTRAP-VALIDATOR-VALIDATOR-001
    ...
    def _check_constraints(self):
        ...
        project_max = self.project_profile.get("constraints", {}).get(
            "max_lines_changed"
        )
        ...
        project_patch = self.project_profile.get("constraints", {}).get(
            "patch_only", True
        )
    ...
    def _check_consistency(self):
        ...
        available_tool_ids = set(
            tool.get("tool_id")
            for tool in self.project_profile.get("available_tools", [])
    ...
    def _auto_fix_common_issues(self):
        ...
        if fixed_paths:
            ...
            with open(self.project_profile_path, "w", encoding="utf-8") as f:
                yaml.dump(
                    self.project_profile, f, default_flow_style=False, sort_keys=False
                )

        # Add missing default values
        if "constraints" not in self.project_profile:
            self.project_profile["constraints"] = {
                "patch_only": True,
...
def main():
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python validator.py <project_profile.yaml> <router_config.json> <profile_id>"
        )
        sys.exit(1)
