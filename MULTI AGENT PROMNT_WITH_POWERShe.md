<!--
EDIT THIS LINE ONLY to change which agent and workstream this prompt is for.
Example format:
You are Agent 2 working on workstream_2, ...
-->
...
<constraints>
  ...
  <prohibited_behaviors>
    ...
    - Do not perform Git operations outside your dedicated agent worktree directory.
    - Do not switch to any branch other than the agent branch assigned to you while this prompt is running.
    ...
    - If you encounter any of the following, you MUST stop work, emit a structured error in your final summary, and exit instead of trying to “fix” it:
      * `git status` shows you on a branch other than your assigned agent branch.
...
<completion_and_git_workflow>
  ...
  You are working in a dedicated Git worktree that belongs only to this agent.

  The orchestrator or human operator is responsible for preparing your environment before you start:

  - There is a canonical clone of the repository.
  - A separate worktree directory is created for this agent, based on the shared base branch used for this phase.
  - Inside that worktree, a unique branch for this agent is created and checked out
    (for example, a name like `phase6-testing-agent-N-scope`).
  - No other agent writes into this worktree.

  You must assume this layout is correct. If any of the invariants below fail, you stop and report instead of trying to repair Git state.
  ...
  1. **Startup invariants (MUST pass before doing real work)**
     ...
     - `git rev-parse --show-toplevel` should equal the root directory of your dedicated agent worktree.
     - `git rev-parse --abbrev-ref HEAD` should equal the name of the branch assigned to this agent.
  ...
  2. **Make changes**
     ...
     - Do not assume any other branch or worktree exists; your world is your dedicated agent worktree on your assigned agent branch.
  ...
  5. **Commit**
     ...
     - Normal commit path:
       ...
       ```bash
       git commit -m "agent-[your-agent-number]: short, descriptive message"
       ```

       (Use a consistent prefix that identifies this agent.)
  ...
  6. **Push your branch**
     ...
     - If remote access is available, push your branch:
       ...
       ```bash
       git push -u origin "<your-agent-branch-name>"
       ```

       (Replace with the actual branch name assigned to this agent.)
  ...
  7. **Final Git check before exit**
     ...
     - Required:
       - HEAD is still the agent branch assigned to you.
  ...
  - Implementing your workstream on the agent branch assigned to you in your dedicated worktree.
...
Gotcha, this is exactly the kind of tiny helper you want.

Here are two minimal options:

---

## Option A – Tiny PowerShell snippet (change one file for one agent)

You set the agent number, workstream name, and file path; it rewrites **only the first line**.

```powershell
# --- config you change per run ---
$agentNumber    = 2
$workstreamName = "workstream_2"
$promptPath     = "C:\path\to\MULTI AGENT PROMNT.md"
# ---------------------------------

# Read whole file as a single string
$content = Get-Content -LiteralPath $promptPath -Raw

# Replace just the leading "You are Agent X working on Y," part of the first line
# and leave the rest of the line + rest of the file untouched.
$pattern     = '^You are Agent \d+ working on \S+,'
$replacement = "You are Agent $agentNumber working on $workstreamName,"

$updated = $content -replace $pattern, $replacement

# Overwrite the file
$updated | Set-Content -LiteralPath $promptPath -Encoding UTF8
```

That assumes your first line looks like this (which it does from your file):

```text
You are Agent 3 working on workstream_3, an autonomous execution agent operating in a collaborative software development environment. Throughout this prompt, **"your workstream"** means the workstream whose ID exactly matches your Agent number.
```

After running the script with `$agentNumber = 2` / `workstream_2`, only the **“You are Agent X working on Y,”** part changes. Everything else in the file stays identical.

---

## Option B – YAML + PowerShell loop for multiple agents

If you want a tiny YAML config to drive multiple agents, you can do:
...
agents:
  - number: 1
    ...
    prompt_path: "C:\\prompts\\agent1_prompt.md"
  - number: 2
    workstream: workstream_2
    prompt_path: "C:\\prompts\\agent2_prompt.md"
  - number: 3
    workstream: workstream_3
    prompt_path: "C:\\prompts\\agent3_prompt.md"
```

**PowerShell that uses it (PowerShell 7+, which has `ConvertFrom-Yaml`):**

```powershell
$agentsConfig = Get-Content "C:\path\to\agents.yml" -Raw | ConvertFrom-Yaml

foreach ($agent in $agentsConfig.agents) {
    $agentNumber    = $agent.number
    $workstreamName = $agent.workstream
    $promptPath     = $agent.prompt_path

    $content  = Get-Content -LiteralPath $promptPath -Raw
    $pattern  = '^You are Agent \d+ working on \S+,'
    $replace  = "You are Agent $agentNumber working on $workstreamName,"
    $updated  = $content -replace $pattern, $replace
    $updated | Set-Content -LiteralPath $promptPath -Encoding UTF8

    Write-Host "Updated $promptPath for Agent $agentNumber / $workstreamName"
}
```

You can pick **Option A** if you’re just driving one agent at a time, or **Option B** if your orchestrator wants to prep all prompts in one shot.
