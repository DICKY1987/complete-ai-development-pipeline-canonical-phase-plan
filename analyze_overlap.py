...
def main():
    ...
    for folder, reason in DEPRECATED_FOLDERS.items():
        if os.path.exists(folder):
            ...
            print(f'   Python: {stats["python_files"]}, Markdown: {stats["markdown_files"]}')
            print()
    ...
    for folder, info in report['deprecated_folders'].items():
        if info['stats']['python_files'] == 0:
            ...
            print(f'✅ SAFE TO ARCHIVE: {folder}/ (no Python code, {info["stats"]["total_files"]} files)')
        else:
            needs_review.append(folder)
            print(f'⚠️  NEEDS REVIEW: {folder}/ ({info["stats"]["python_files"]} Python files)')
    ...
    uet_archived = sum(1 for v in report['uet_overlaps'].values() if not v['old_exists'])
