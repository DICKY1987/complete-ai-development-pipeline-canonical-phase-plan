        ...
        for module in self.all_modules:
            ...
            if not has_tests:
                if is_stale and not is_imported:
                    ...
                    reasons = [
                        "No test coverage",
                        "Deprecated naming pattern detected"
                    ]
    ...
    def generate_report(self, output_path: Path):
        ...
        report = {
            ...
            "test_coverage_scores": {
                module: asdict(score)
                for module, score in sorted(self.scores.items(), key=lambda x: x[1].score, reverse=True)
            },
            "statistics": {
                "total_modules": len(self.all_modules),
                "modules_with_tests": sum(1 for s in self.scores.values() if s.has_tests),
                "modules_without_tests": sum(1 for s in self.scores.values() if not s.has_tests),
            ...
            "coverage_gaps": [
                module for module, score in self.scores.items()
        ...
        print(f"Modules without tests: {report['statistics']['modules_without_tests']:,}")
        print(f"\nArchival candidates (by score):")
        print(f"  Score 95+ (no tests + stale + isolated): {report['statistics']['score_95_plus']:,}")
        print(f"  Score 80+ (no tests + deprecated):       {report['statistics']['score_80_plus']:,}")
        print(f"  Score 70+ (no tests + isolated):         {report['statistics']['score_70_plus']:,}")
        print(f"  Score 50+ (no tests):                    {report['statistics']['score_50_plus']:,}")
        print(f"\nReport: {output_path}")
        print("="*70 + "\n")
