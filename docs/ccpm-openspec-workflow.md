...
### 3. Generate Workstream JSON

```bash
# Convert bundle/change to a pipeline workstream
python scripts/generate_workstreams_from_openspec.py \
  --change-id <change-id> \
  --files-scope <path1> <path2> ... \
  --tool aider \
  --gate 1 \
  --ccpm-issue TBD

# Output location
cat workstreams/ws-<change-id>.json
```

### 4. Create CCPM Epic (Optional)
...
### 6. Run Pipeline Validation

```bash
# Validate workstreams first (schema, overlaps)
python ./scripts/validate_workstreams.py

# Option A: Workstream runner (recommended)
python ./scripts/run_workstream.py --ws-id ws-<change-id>

# Option B: Parallel (multiple workstreams)
python -c "from src.orchestrator import parallel; print(parallel.run_many(['ws-<id1>','ws-<id2>'], max_workers=2))"
```

### 7. Track Progress with CCPM
...
### OpenSpec  Workstream Converter
**Tool:** `scripts/generate_workstreams_from_openspec.py`

**Function:** Converts an OpenSpec change or bundle into a schema-valid workstream
