...
This phase replaces the original DOC_ID rollout process (multiple worktrees writing directly to `DOC_ID_REGISTRY.yaml`, reruns after CLI crashes, manual index updates) with a **deterministic, tool-driven workflow** based on:
...
3. `DOC_ID_REGISTRY.yaml` validates against the new schema.  
...
* Multiple worktrees wrote directly to `DOC_ID_REGISTRY.yaml`.
...
2. Create a batch spec:
   ...
   ```yaml
   # docid_batches/docid_batch_modules_pipeline.yaml
...
3. Run `batch-mint` in dry-run mode:
   ...
   python doc_id_registry_cli.py \
     ...
     batch-mint \
     --batch docid_batches/docid_batch_modules_pipeline.yaml \
     --mode dry-run \
     --dry-run-report docid_reports/preview_PIPELINE_001.md
   ```
...
4. Run `batch-mint` in deltas-only mode:
   ...
   python doc_id_registry_cli.py \
     ...
     --batch docid_batches/docid_batch_modules_pipeline.yaml \
     --mode deltas-only \
     --delta-out docid_deltas/delta_DOCID_PIPELINE_001.jsonl \
     --no-registry
   ```
...
5. Merge delta on the control checkout:
   ...
   python doc_id_registry_cli.py \
     --registry DOC_ID_REGISTRY.yaml \
     merge-deltas \
     docid_deltas/delta_DOCID_PIPELINE_001.jsonl \
     --report docid_reports/merge_DOCID_PIPELINE_001.md
   ```
...
6. Validate:
   ...
   python doc_id_registry_cli.py \
     --registry DOC_ID_REGISTRY.yaml \
     validate --duplicates \
     --report docid_reports/final_validation_PIPELINE_001.md
   ```
...
* Only the control checkout:
  ...
  * Runs `merge-deltas` and writes `DOC_ID_REGISTRY.yaml`.

**Steps:**

1. Define/extend a batch spec in `docid_batches/`.
...
2. Merge deltas:
   ...
   python doc_id_registry_cli.py \
     --registry DOC_ID_REGISTRY.yaml \
     merge-deltas \
     docid_deltas/delta_*.jsonl \
     --report docid_reports/merge_DOCID_ALL_001.md
   ```
...
4. If merge is good:
   ...
   * Commit `docid_reports/merge_DOCID_ALL_001.md`,
...
* Worktrees MUST NOT:
  ...
  * Run `merge-deltas`.
  * Write `DOC_ID_REGISTRY.yaml` directly.
...
3. `DOC_ID_REGISTRY.yaml` conforms to the new schema (Pydantic model).
...
5. Old habits have been eliminated:
   ...
   * No worktree directly edits `DOC_ID_REGISTRY.yaml`.
