...
class ContractViolationError(Exception):
    ...
    def __init__(self, result: ValidationResult):
        self.result = result
        messages = [v.message for v in result.violations if v.severity == Severity.ERROR]
            ...
            if not result.valid:
                ...
                for violation in result.violations:
                    logger.error(
                        ...
                        + (f" -> {violation.remediation}" if violation.remediation else "")
                    )
...
def validate_schema(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not result.valid:
                error_msg = f"Schema validation failed for {schema_name}.{schema_version}"
                logger.error(error_msg)
...
def create_phase_decorator(
    ...
    Example:
        ...
        def finish_request_building():
            pass
    """
    entry_dec = functools.partial(enforce_entry_contract, phase=phase, validator=validator)
    exit_dec = functools.partial(enforce_exit_contract, phase=phase, validator=validator)
