...
class CentralityAnalyzer:
    ...
    def rank_modules(
        self, metric: str = "pagerank", top_n: int = None
    ...
    def identify_core_modules(self, threshold_percentile: float = 0.8) -> List[str]:
        ...
        for module, m in metrics.items():
            ...
            combined = (
                ...
                + (
                    m["in_degree"]
                    / max(1, max(metrics[mod]["in_degree"] for mod in metrics))
                )
                * 0.3
                + (
                    m["total_degree"]
                    / max(1, max(metrics[mod]["total_degree"] for mod in metrics))
                )
                * 0.2
            )
        ...
        core_modules = [
            module for module, score in combined_scores.items() if score >= threshold
        ]

        return sorted(core_modules)

    def export_ranking(
        self, output_path: Path, metric: str = "pagerank", top_n: int = 50
    ):
        """
        ...
        data = {
            ...
            "statistics": {
                ...
                "core_percentage": round(
                    len(core_modules) / max(1, len(self.graph.get_all_modules())) * 100,
                    1,
                ),
            },
        }
