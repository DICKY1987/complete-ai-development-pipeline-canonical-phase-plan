...
class CodexAdapter:
    """Adapter for executing GitHub Copilot CLI (codex) jobs."""
    ...
    def run_job(self, job: Dict[str, Any]) -> JobResult:
        ...
        try:
            with open(log_file, "w", encoding="utf-8") as log:
                log.write(f"=== Codex Job: {job['job_id']} ===\n")
    ...
    def validate_job(self, job: Dict[str, Any]) -> bool:
        """Validate that job has required fields for Codex."""
    ...
    def get_tool_info(self) -> Dict[str, Any]:
        """Get Codex tool metadata."""
        return {
            "tool": "codex",
            "adapter_version": "0.1.0",
            "capabilities": ["code_review", "code_generation", "escalation"],
            "requires_env": []  # Codex uses system auth
        }
    ...
    def _write_error_report(self, path: Path, job: Dict[str, Any], 
        ...
        report = {
            ...
            "tool": "codex",
            "exit_code": exit_code,
            "summary": "Codex execution failed" if exit_code > 0 else "Codex timeout/error",
            "details": stderr[:20],
