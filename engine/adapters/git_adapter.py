...
- Execute git operations (commit, worktree, branch, etc.).
...
USAGE:
- Automated git operations in pipeline
- Worktree management
...
class GitAdapter:
    """Adapter for executing git operation jobs."""
    ...
    def run_job(self, job: Dict[str, Any]) -> JobResult:
        ...
        try:
            with open(log_file, "w", encoding="utf-8") as log:
                log.write(f"=== Git Job: {job['job_id']} ===\n")
            ...
            exit_code = -1
            stderr_lines = [f"Git operation timeout after {e.timeout} seconds"]
    ...
    def validate_job(self, job: Dict[str, Any]) -> bool:
        """Validate that job has required fields for git."""
    ...
    def _build_env(self, job: Dict[str, Any]) -> Dict[str, str]:
        ...
        # Ensure git operates non-interactively
    ...
    def _write_error_report(self, path: Path, job: Dict[str, Any], 
        ...
        report = {
            ...
            "tool": "git",
            "exit_code": exit_code,
            "summary": "Git operation failed" if exit_code > 0 else "Git timeout/error",
            "details": stderr[:20],
            "workstream_id": job["workstream_id"],
            "git_command": job["command"]["args"][0] if job["command"]["args"] else "unknown"
        }
...
def run_git_job(job: Dict[str, Any]) -> JobResult:
    """
