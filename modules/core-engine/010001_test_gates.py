...
class TestGateEnforcer:
    ...
    def enforce_gates(self, workstream_id: str, run_id: str, context: Dict[str, Any]) -> List[GateResult]:
    ...
    def _execute_gate(self, gate: TestGate, workstream_id: str, run_id: str, context: Dict[str, Any]) -> GateResult:
        """Execute single gate.
        ...
        Returns:
            GateResult
        """
        from modules.core_engine import tools
        from modules.core_state import db
        executed_at = datetime.now(timezone.utc)
        try:
            tool_result = tools.run_tool(gate.tool_id, context, run_id=run_id, ws_id=workstream_id)
            passed = bool(tool_result.success)
            error_msg = None if passed else str(tool_result.error)
            db.record_event(event_type='test_gate_executed', run_id=run_id, ws_id=workstream_id, payload={'gate_id': gate.gate_id, 'gate_type': gate.gate_type, 'passed': passed, 'blocking': gate.blocking})
            return GateResult(gate_id=gate.gate_id, passed=passed, workstream_id=workstream_id, error_message=error_msg, executed_at=executed_at)
        except Exception as e:
            db.record_event(event_type='test_gate_error', run_id=run_id, ws_id=workstream_id, payload={'gate_id': gate.gate_id, 'error': str(e)})
            return GateResult(gate_id=gate.gate_id, passed=False, workstream_id=workstream_id, error_message=str(e), executed_at=executed_at)

    def enforce_wave_gates(self, wave_workstreams: List[str], run_id: str, context: Dict[str, Any]) -> Dict[str, List[GateResult]]:
...
def create_default_gates() -> List[TestGate]:
    ...
    """
    return [TestGate(gate_id='syntax-check', gate_type='static', tool_id='python-syntax-check', required=True, blocking=True, wave_boundary=False), TestGate(gate_id='lint', gate_type='static', tool_id='ruff', required=False, blocking=False, wave_boundary=False), TestGate(gate_id='unit-tests', gate_type='runtime', tool_id='pytest', required=True, blocking=True, wave_boundary=True)]
